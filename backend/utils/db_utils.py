...
from backend.utils.db_system_prompt_actions import DbSystemPromptActions
from backend.utils.db_game_session_actions import DbGameSessionActions
from backend.utils.db_chat_actions import DbChatActions
from backend.utils.db_visualization_task_actions import DbVisualizationTaskActions
...
class DatabaseManager:
    ...
    def add_image_url_to_visual_prompt(
        self, session_id: str, message_id: str, prompt: str, image_url: str
    ) -> bool:
        """Persist an image URL against a specific visual prompt for a message."""
        if not self.chat_actions:
            self.logger.error("Chat actions not available - database not connected")
            return False
